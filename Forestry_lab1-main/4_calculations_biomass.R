...
#----------------
Biomass <- read.csv("https://raw.githubusercontent.com/kilmert02/Forestry_Lab_Data/refs/heads/main/Biomass%20Equation.csv")
Bm_equa <- Biomass %>% select(b0, b1, Chojnacky_Code)
#----------------
...
#----------------
trees <- trees %>% filter(b0 != 0) %>% filter(b1 !=0)
#----------------
...
#----------------
sum_u2_bm <- trees %>% group_by(Plot) %>% summarise(sum_biomass = sum(biomass))
#----------------
...
#----------------
sum_u2_bm <- sum_u2_bm %>% mutate(bm_pa = sum_biomass * EF)
#----------------
